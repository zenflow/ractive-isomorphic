...
var initialise = function(self, options, Super){
	...
	// expose api
	//self.api = _.clone(options.api || self.api || {});
	//delete options.api;
	...
	options.partials = _.assign({}, options.partials || {}, {
		content: _.map(self.pages, function(Page, page_name){
			return '{{#route=="'+page_name+'"}}<rx-'+page_name+' route="'+page_name+'" params="{{params}}" />{{/}}';
		}).join('')
	});
