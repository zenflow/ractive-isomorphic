...
var initialise = function(self, options, Super){
	self._assertConstruct();

	// expose pages
	self.pages = _.clone(options.pages || self.pages || {});
	delete options.pages; // we set self.pages already; dont override it with the Super call

	// throws ri.errors.NoMatchingUrlError
	self._initRouter(options.url);
	delete options.url;
